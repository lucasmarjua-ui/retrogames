...
function reset() { clearInterval(timer); recordGameStart('pacman'); score = 0; pelletsEaten = 0; fruitSpawns = 0; level = 1; running = false; paused = false; loadMap(); hud.setScore(score); bestElement.textContent = String(getBestScore('pacman')).padStart(4, '0'); statusElement.textContent = 'PULSA UNA FLECHA'; draw(); }
...
function endGame(won = false) { clearInterval(timer); running = false; if (won) unlockAchievement('pacman-perfect'); const result = saveScore('pacman', score, { bronze: 400, silver: 1000, gold: 2000 }); submitScore('pacman', score).catch(() => {}); recordGameEnd('pacman'); Wallet.add(Math.max(1, Math.floor(score / 100))); Audio.playSfx('coin'); Audio.playSfx(won ? 'levelup' : 'gameover'); bestElement.textContent = String(result.bestScore).padStart(4, '0'); statusElement.textContent = won ? `LABERINTO LIMPIO / ${result.medal ? result.medal.toUpperCase() : 'SIN MEDALLA'}` : `GAME OVER / ${result.medal ? result.medal.toUpperCase() : 'SIN MEDALLA'}`; draw(); }
