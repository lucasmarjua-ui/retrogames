...
function tick() { if (canMove(pacman, nextDirection)) direction = nextDirection; if (canMove(pacman, direction)) { pacman.x += direction.x; pacman.y += direction.y; } const pellet = key(pacman.x, pacman.y); if (pellets.has(pellet)) { pellets.delete(pellet); const power = layout[pacman.y][pacman.x] === 'o'; score += power ? 50 : 10; if (power) powerUntil = Date.now() + 7000; hud.setScore(score); } ghosts.forEach(ghost => moveGhost(ghost)); const caught = ghosts.find(ghost => ghost.x === pacman.x && ghost.y === pacman.y); if (caught) { if (Date.now() < powerUntil) { score += 200; caught.x = 10; caught.y = 9; hud.setScore(score); } else return endGame(); } if (!pellets.size) return endGame(true); draw(); }
...
function endGame(won = false) { clearInterval(timer); running = false; const result = saveScore('pacman', score, { bronze: 400, silver: 1000, gold: 2000 }); Wallet.add(Math.max(1, Math.floor(score / 100))); bestElement.textContent = String(result.bestScore).padStart(4, '0'); statusElement.textContent = won ? `LABERINTO LIMPIO / ${result.medal ? result.medal.toUpperCase() : 'SIN MEDALLA'}` : `GAME OVER / ${result.medal ? result.medal.toUpperCase() : 'SIN MEDALLA'}`; draw(); }
