...
function finishGameOver() { if (startedAt && Date.now() - startedAt >= 60000) unlockAchievement('snake-survivor'); const result = saveScore('snake', score, { bronze: 50, silver: 120, gold: 250 }); submitScore('snake', score).catch(() => {}); recordGameEnd('snake'); Wallet.add(Math.max(1, Math.floor(score / 20))); Audio.playSfx('coin'); Audio.playSfx('gameover'); bestElement.textContent = String(result.bestScore).padStart(4, '0'); statusElement.textContent = `GAME OVER / ${result.medal ? result.medal.toUpperCase() : 'SIN MEDALLA'}`; draw(); }
