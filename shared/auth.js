...
const DATA_KEYS = ['retrogames.wallet', 'retrogames.game-data', 'retrogames.unlocks', 'retrogames.achievements', 'retrogames.streak'];
...
function mergeData(local, cloud) {
  ...
  ['retrogames.game-data', 'retrogames.unlocks', 'retrogames.achievements', 'retrogames.streak'].forEach(key => {
    try {
      const localValue = JSON.parse(local[key] || '{}');
      const cloudValue = JSON.parse(cloud[key] || '{}');
      merged[key] = JSON.stringify(key === 'retrogames.game-data' ? mergeGameData(localValue, cloudValue) : key === 'retrogames.unlocks' ? mergeUnlocks(localValue, cloudValue) : key === 'retrogames.achievements' ? mergeAchievements(localValue, cloudValue) : mergeStreak(localValue, cloudValue));
